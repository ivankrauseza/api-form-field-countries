...
function displayFilteredResults(results) {
    ...
    if (results.length === 0) {
        filteredData.innerHTML = 'No results found.';
        return;
    }

    results.forEach(item => {
        const resultElement = document.createElement('div');
        resultElement.textContent = `Country Name: ${item.countryName}, Country Code: ${item.countryCode}`;
        filteredData.appendChild(resultElement);
    });
}
